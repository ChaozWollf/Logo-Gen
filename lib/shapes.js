...
class Circle extends Shapes {
    render() {
       return `<circle cx="25" cy="75" r="20" stroke="transparent" fill="${this.color}" stroke-width="5" />`
    }
}

class Square extends Shapes {
    render () {
             return `<rect x="10" y="10" width="30" height="30" stroke="transparent" fill="${shape.color}" stroke-width="5" />`
    }
...
class Triangle extends Shapes {
   render() {
       return `<polygon points="150, 18 244, 182 56, 182" stroke="transparent" fill="${shape.color}" stroke-width="5" />`
    }
}
